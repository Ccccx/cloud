...
//			@Signature(method = "update", type = Executor.class, args = {MappedStatement.class, Object.class}),
	...
	public static class MyInterceptor implements Interceptor {
		...
		public Object intercept(Invocation invocation) throws Throwable {
			...
			final Method getSqlSource = MappedStatement.class.getDeclaredMethod("getSqlSource");
			SqlSource sqlSource = ((SqlSource) getSqlSource.invoke(statement));
			...
			if (SqlCommandType.SELECT.equals(statement.getSqlCommandType())) {
				...
				if (parameter instanceof ParamMap) {
					final ParamMap<Object> map = (ParamMap<Object>) parameter;
					final Optional<Object> optional = Optional.ofNullable(map.get(Constants.WRAPPER));
					if (optional.isPresent()) {
						final Object wrap = optional.get();
						if (wrap instanceof AbstractWrapper) {
							try {
								final AbstractWrapper<?, ?, ?> queryWrap = (AbstractWrapper<?, ?, ?>) wrap;
								final Method getEntityClass = AbstractWrapper.class.getDeclaredMethod("getEntityClass");
								getEntityClass.setAccessible(true);
								final Object invoke = getEntityClass.invoke(queryWrap);
								final Class clz = (Class) invoke;
								final TableName annotation = AnnotationUtils.findAnnotation(clz, TableName.class);
								log.info("{}", annotation);
							} catch (Exception e) {
								e.printStackTrace();
							}
